...
class StreamerDataset(Dataset):
    def __init__(self, root_dir):
        ...
        self.streamers = os.listdir(root_dir)[:5]
        ...
        for streamer in self.streamers:
            ...
            if os.path.exists(json_path) and os.path.exists(metadata_path):
                ...
                for entry in json_data:
                    ...
                    self.data.append({"text": text_path, "audio": audio_path, "metadata": metadata})
    ...
    def extract_audio_features(self, audio_path):
        ...
        inputs = wav2vec_processor(waveform, sampling_rate=sample_rate, return_tensors="pt", padding=True).to(DEVICE)
        with torch.no_grad():
    ...
    def __getitem__(self, idx):
        ...
        metadata_features = [entry["metadata"].get(k, 0) for k in meta_keys]
        metadata_features = torch.tensor(metadata_features, dtype=torch.float32)

        return text_features.squeeze(0), audio_features.squeeze(0), metadata_features
...
class MultiModalMLP(nn.Module):
    ...
    def forward(self, text_input, audio_input, meta_input):
        text_features = self.text_mlp(text_input)
        audio_features = self.audio_mlp(audio_input)
        meta_features = self.meta_mlp(meta_input)
        fused = torch.cat((text_features, audio_features, meta_features), dim=1)
...
def train_model(model, dataloader, epochs=10, lr=1e-4, device="cuda"):
    ...
    for epoch in range(epochs):
        ...
        for text_features, audio_features, metadata_features in dataloader:
            ...
            optimizer.zero_grad()
            outputs = model(text_features, audio_features, metadata_features)
            loss = criterion(outputs.squeeze(), metadata_features[:, -1])  # Predicting "Followers"
            loss.backward()
            optimizer.step()

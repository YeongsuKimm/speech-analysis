    ...
    def __init__(self, root_dir, label_dict):
        ...
        for streamer in self.streamers:
            if streamer not in self.label_dict:
                continue  # skip streamers with no label
...
def train_model(model, tdataloader, vdataloader, optimizer, criterion, num_epochs=10):
    model.to(DEVICE)
    ...
    for epoch in range(num_epochs):
        ...
        # Training loop
        for text, audio, labels in tdataloader:
        ...
        train_loss = total_loss / len(tdataloader)
        ...
        with torch.no_grad():
            for text, audio, labels in vdataloader:
                text, audio, labels = text.to(DEVICE), audio.to(DEVICE), labels.to(DEVICE)
...
if __name__ == "__main__":
    ...
    tdataset = StreamerDataset(root_dir="processed", label_dict=label_dict)
    tdataloader = DataLoader(tdataset, batch_size=16, shuffle=True)
    ...
    streamer_list = []
    for streamer in os.listdir(".test"):
        streamer_list.append(streamer)
    label_dict = get_dict(streamer_list)

    vdataset = StreamerDataset(root_dir=".test", label_dict=label_dict)
    vdataloader = DataLoader(tdataset, batch_size=16, shuffle=True)

    model = MultiModalMLP(text_dim=768, audio_dim=768, hidden_dim=128, output_dim=3)
    optimizer = optim.Adam(model.parameters(), lr=1e-2)
    criterion = nn.CrossEntropyLoss()

    train_model(model, tdataloader, vdataloader, optimizer, criterion, num_epochs=30)
    model_path = f"models/streamer_class_model_t60.pth"
